...
import { NovedadesService, Novedad, CreateNovedadRequest, EmpleadoOption } from './services/cargue.service';
...
@Component({
  ...
  styleUrls: ['./cargue.component.css']
})
export class CargueEventosComponent implements OnInit {
  ...
  loadNovedades(): void {
    ...
    const endpoint = estado
      ? this.novedadesService.getNovedadesPorEstado(estado)
      : this.novedadesService.getNovedadesPorEstado('');
  ...
  aplicarFiltros(): void {
    ...
    if (this.selectedEstado) {
      result = result.filter(n => n.estado === this.selectedEstado);
    }
